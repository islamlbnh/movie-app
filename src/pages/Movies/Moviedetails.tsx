...
export const MovieDetails = () => {
  ...
  return (
    <div
      className="md:h-screen h-full w-[99vw]  flex items-center justify-center py-12"
      style={{
    ...
    >
      ...
      <div className="relative w-fit p-6 h-full bg-[#211212] shadow-lg rounded-lg  flex flex-col md:flex-row gap-8 z-10">
        <img
        ...
        <div className="flex-1 flex flex-col gap-3">
          ...
          <p className="italic text-gray-400 py-4">{data.data.tagline}</p>
          <p className="text-gray-200 py-4">{data.data.overview}</p>
          <div className="flex flex-wrap gap-2 my-2">
            {data.data.genres.map((g: any) => (
              <span
                key={g.id}
                className="bg-[#3a2323] text-red-200 px-2 py-4 rounded text-xs font-medium"
              >
                {g.name}
              </span>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 pt-4 text-sm text-gray-300">
